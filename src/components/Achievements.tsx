      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <Tabs defaultValue="publications" className="w-full">
          ...
          <TabsContent value="volunteer" className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {volunteerExperiences.map((exp, index) => <Card key={index} className="glass-card hover:shadow-lg transition-shadow">
                <CardHeader>
                  <div className="flex items-start gap-3 mb-2">
                    ...
                    <div>
                      ...
                      <CardDescription className="font-semibold text-foreground mt-1">
                        {exp.url ? <a href={exp.url} target="_blank" rel="noopener noreferrer" className="hover:text-primary transition-colors underline">
                            {exp.organization}
                          </a> : exp.organization}
                      </CardDescription>
                ...
                <CardContent>
                  <ul className="space-y-2 text-sm text-muted-foreground">
                    {exp.highlights.map((highlight, idx) => <li key={idx} className="flex gap-2">
          ...
          <TabsContent value="leadership" className="grid gap-6 md:grid-cols-2">
            {organizations.map((org, index) => <Card key={index} className="glass-card hover:shadow-lg transition-shadow">
                <CardHeader>
                ...
                <CardContent>
                  <ul className="space-y-2 text-sm text-muted-foreground">
                    {org.highlights.map((highlight, idx) => <li key={idx} className="flex gap-2">
                        <span className="text-primary mt-1">•</span>
                        <span>{highlight}</span>
                      </li>)}
                  </ul>
